        ...
        if (!stat) {
                                        ...
                                        message.awaitReactions(filter3, {
                                            ...
                                            .then(collected => {
                                                ...
                                                Promise.all(jimps).then(function(data) {
                                                    ...
                                                    jimp.loadFont("src/fonts/Eutemia/Eutemia.fnt").then(font => {
                                                        /// +140
                                                        data[0].print(font, 0, 0, `${stat.class}`)
                                                ...
                                                msg.channel.send("**Creating profile...**").then(m => {
                                                    setTimeout(() => {
                                                        m.delete().then(() => msg.channel.send(`**${msg.author.username}**'s profile card`, new Discord.MessageAttachment("Profiles/" + msg.author.username + ".png"))).then(() => {
                                                            setTimeout(() => {
            ...
            if (!msg.mentions.users.first()) {
                ...
                Promise.all(jimps).then(function(data) {
                    ...
                    data[0].composite(data[1], 55, 80)
                    jimp.loadFont("src/fonts/Eutemia/Eutemia.fnt").then(font => {
                        /// +30
                        data[0].print(font, 70, 275, `${stat.class}`)
                            .print(font, 60, 55, `Race: ${stat.race}`)
                            .print(font, 225, 40, `Atk: 0`)
                            .print(font, 225, 70, `Def: 0`)
                            .print(font, 225, 100, `Hp: ${stat.hp}`)
                            .print(font, 225, 130, `Mana: ${stat.mana}`)
                            .print(font, 225, 160, `Stamina: ${stat.stamina}`)
                            .print(font, 225, 190, `Abilities:`)
                            .print(font, 225, 270, `Items:`)
                            .write("Profiles/" + msg.author.username + '.png');
                    })
                });
                msg.channel.send("**Loading profile...**").then(m => {
                    setTimeout(() => {
                        m.delete().then(() => msg.channel.send(`**${msg.author.username}**'s profile card`, new Discord.MessageAttachment("Profiles/" + msg.author.username + ".png"))).then(() => {
                            setTimeout(() => {
                                fs.unlinkSync("Profiles/" + msg.author.username + '.png');
                ...
                let statM = this.client.getStats.get(msg.mentions.users.first().id, pi);
                var images = ["src/profile/profile.png", `src/race/${statM.race}/${statM.gender}/${statM.class}.png`];
                var jimps = [];
                for (var i = 0; i < images.length; i++) {
                    jimps.push(jimp.read(images[i]));
                }
                Promise.all(jimps).then(function(data) {
                    ...
                    jimp.loadFont("src/fonts/Eutemia/Eutemia.fnt").then(font => {
                        /// +30
                        data[0].print(font, 70, 275, `${statM.class}`)
                            .print(font, 60, 55, `Race: ${statM.race}`)
                            .print(font, 225, 40, `ATK: 0`)
                            .print(font, 225, 70, `DEF: 0`)
                            .print(font, 225, 100, `HP: ${statM.hp}`)
                            .print(font, 225, 130, `Mana: ${statM.mana}`)
                            .print(font, 225, 160, `Stamina: ${statM.stamina}`)
                            .print(font, 225, 190, `Abilities:`)
                            .print(font, 225, 270, `Items:`)
                            .write("Profiles/" + msg.author.username + '.png');
                    })
